...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for bitcoind is necessary for this upgrade with the "reindex=1" tealcoin.conf option. \n' +
                 'There are changes necessary in both tealcoin.conf and litecore-tealcoin-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/tealcoind')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed litecore-tealcoin-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './litecore-tealcoin-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in litecore-tealcoin-node services directory
