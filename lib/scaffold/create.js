...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'litecore-tealcoin-lib': '^' + bitcore.version,
    'litecore-tealcoin-node': version
  }
};

/**
 * Will create a directory and tealcoin.conf file for Bitcoin.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/litecore-tealcoin-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the litecore-tealcoin-node directory and configuration
