...
var BitcoinRPC = require('tealcoind-rpc');
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './tealcoin.conf');

  if (this.node.network === bitcore.Networks.testnet) {
	if (!fs.existsSync(spawnOptions.datadir + '/testnet4')) {
		mkdirp.sync(spawnOptions.datadir + '/testnet4');
	}
	var configPath = path.resolve(spawnOptions.datadir, './testnet4/tealcoin.conf');
  }
  if (this.node.network === bitcore.Networks.regtest) {
	if (!fs.existsSync(spawnOptions.datadir + '/regtest')) {
		mkdirp.sync(spawnOptions.datadir + '/regtest');
	}
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of litecore-tealcoin-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of litecore-tealcoin-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of litecore-tealcoin-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in tealcoin.conf'
  );
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of litecore-tealcoin-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getDefaultConf = function() {
  ...
  if (this.node.network === bitcore.Networks.testnet) {
    networkOptions.rpcport = 19992;
  }
  if (this.node.network === bitcore.Networks.regtest) {
    networkOptions.rpcport = 29992;
  }
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet4/tealcoin.conf';
  }
  if (this.node.network === bitcore.Networks.regtest) {
    networkPath = 'regtest/tealcoin.conf';
  }
  return networkPath;
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/tealcoind.pid';
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new BitcoinRPC({
        protocol: 'http',
        host: '127.0.0.1',
        port: self.spawn.config.rpcport || 9992,
